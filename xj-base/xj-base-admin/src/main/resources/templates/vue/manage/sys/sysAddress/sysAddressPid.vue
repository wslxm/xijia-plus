    ...
    methods: {
        ...
        init(newRowData) {
            ...
            this.crud.get(this.uri.infoTree).then((res) => {
                ...
                // 默认选中父级
                this.$refs.crudSysAddressPid.toggleSelection([this.checkedRow]);
                // 当前行+下级的ids 禁止选择
        ...
        select(selection, row) {
            this.checkedRow = row;
            this.$refs.crudSysAddressPid.toggleSelection();
            this.$refs.crudSysAddressPid.toggleSelection([row]);
        },
        // 提交修改请求
        updPid() {
