package com.github.wslxm.springbootplus2.basepay.manage.model.vo;
...
@ApiModel(value = "PayRecordVO 对象", description = "第三方支付记录表")
public class PayRecordVO extends BaseVo {

    private static final long serialVersionUID = -506257672325173258L;

    @ApiModelProperty(notes = "订单号(对应订单表)", position = 0)
    private String orderNo;

    @ApiModelProperty(notes = "交易号(第三方交易时生成 支付/退款/放款等)", position = 1)
    private String tradeNo;

    @ApiModelProperty(notes = "平台手续费(元)", position = 2)
    private BigDecimal platformFee;

    @ApiModelProperty(notes = "渠道手续费(元), 为0时,手续费累加在 platform_fee", position = 3)
