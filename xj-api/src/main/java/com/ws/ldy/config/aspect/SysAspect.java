...
import com.ws.ldy.common.idempotent.aop.IdempotentAspect;
...
//    private HttpServletResponse response;
    ...
    private Object run(ProceedingJoinPoint proceed) throws Throwable {
        ...
        // new 3.1、幂等验证
        R apiIdempotentR = IdempotentAspect.run(proceed, request, response);
    ...
    private void printLog(String ip, String host, Integer port, String className, String url, Object[] args, String classDesc, String methodDesc) {
        ...
        log.info("" +
                        ...
                        "\r\n| URL: [{}]   PARAM: [{}] " +
                        "\r\n|------------------------------------------------------------------------------------------------------------------------------------------------------------------|",
                ip, host, port,  classDesc, methodDesc,className,
                String.format("%-65s", url), args
        );
    }
    ...
    public R blacklistAuth(String ip) {
        ...
        // 如果没有缓存，就去数据库获取
        if (BaseCache.BLACKLIST_CACHE == null) {
            // 如果数据库没有配置，缓存设置默认对象，让其不为空，防止无限制查询数据库
            BaseCache.BLACKLIST_CACHE = new HashMap<>();
            ...
            if (blacklist.size() > 0) {
                ...
                if (baiMD != null) {
                    ...
                    BaseCache.BLACKLIST_CACHE.put(Enums.Admin.BlacklistType.BLACKLIST_TYPE_1.getValue(), baiIps);
                }
                if (heiMD != null) {
                    List<String> heiIps = heiMD.stream().map(p -> p.getIp()).collect(Collectors.toList());
                    BaseCache.BLACKLIST_CACHE.put(Enums.Admin.BlacklistType.BLACKLIST_TYPE_2.getValue(), heiIps);
                }
            }
        }
        // 1、没有配置黑/白名单，直接放行
        if (BaseCache.BLACKLIST_CACHE.size() == 0) {
            return R.success();
        }
        // 2、没有配置黑名单，直接放行
        if (!BaseCache.BLACKLIST_CACHE.containsKey(Enums.Admin.BlacklistType.BLACKLIST_TYPE_2.getValue())) {
            return R.success();
        }
        // 3、检查黑名单，如果被列进的黑名单，直接拦截
        if (BaseCache.BLACKLIST_CACHE.containsKey(Enums.Admin.BlacklistType.BLACKLIST_TYPE_2.getValue())) {
            List<String> heiIps = BaseCache.BLACKLIST_CACHE.get(Enums.Admin.BlacklistType.BLACKLIST_TYPE_2.getValue());
            if (heiIps.contains(ip)) {
                return R.error(RType.SYS_BLACK_LIST_IP.getValue(), "[" + ip + "] " + RType.SYS_BLACK_LIST_IP);
            }
        }
        // 4、如果配置了白名单( * )直接放行除了黑名单的所有ip
        if (BaseCache.BLACKLIST_CACHE.containsKey(Enums.Admin.BlacklistType.BLACKLIST_TYPE_1.getValue())) {
            List<String> baiIps = BaseCache.BLACKLIST_CACHE.get(Enums.Admin.BlacklistType.BLACKLIST_TYPE_1.getValue());
            if (baiIps.contains("*")) {
                return R.success();
            }
        }
