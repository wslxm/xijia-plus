...
import com.ws.ldy.modules.yw.caipu.service.CpCategoryService;
 ...
 */
@RestController
@RequestMapping(BaseConstant.Uri.apiAdmin + "/caipu/cpCategory")
@Api(value = "CpCategory", tags = "独立功能--菜谱类别字典表")
public class CpCategoryController extends BaseController<CpCategoryService> {
    ...
    public R<List<CpCategoryVO>> findList(Boolean isBottomLayer) {
        ...
        if (isBottomLayer == null || isBottomLayer) {
            ...
            List<CpCategory> newList = list.stream().filter(i -> !isInteger(i.getCode())).collect(Collectors.toList());
...
//    }
    ...
    public R<Void> insert(@RequestBody com.ws.ldy.modules.sys.admin.model.dto.AdminDictionaryDTO dto) {
        ...
        if (!isInteger(dto.getCode()) && baseService.count(new LambdaQueryWrapper<CpCategory>().eq(CpCategory::getCode, dto.getCode())) > 0) {
