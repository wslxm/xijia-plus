...
export default {
    components: {
        Add: () => import('./depAdd'),
        Upd: () => import('./depUpd'),
        UpdPid: () => import('./depPid')
    },
    data() {
        return {
            treeOption: JSON.parse(JSON.stringify(this.website.treeOption)),
            uri: {
                infoTree: "/api/admin/sys/dep/tree",
                infoList: "/api/admin/sys/dep/findPage",
                info: "/api/admin/sys/dep",
            },
            loading: true,
            dialogWidth: "60%",
    ...
    activated:
        function () {
            this.crud.doLayout(this, this.$refs.cruddep)
        }
    ,
    mounted() {
        // 1、左侧菜单配置
