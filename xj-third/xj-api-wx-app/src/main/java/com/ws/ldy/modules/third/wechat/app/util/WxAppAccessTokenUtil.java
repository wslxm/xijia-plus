...
/**
 * 获取微信小程序的 Token 并缓存 (调用微信相关接口需要Token信息, 注意此 access_token并非网页授权的access_token, 此token 仅用于调用微信相关接口)
 * @author wangsong
...
@Component
@Slf4j
public class WxAppAccessTokenUtil {
    ...
    public R<String> getToken() {
        if (expiration <= System.currentTimeMillis()) {
            String url = WxAppUrl.SubscribeUrl.ACCESS_TOKEN_URL
                    .replace("APPID", wxAppProperties.getAppId())
                    .replace("APPSECRET", wxAppProperties.getSecret());
            // http //222.211.239.190
            ResponseEntity<String> forEntity = restTemplate.getForEntity(url, String.class);
            // 注意微信小程序平台 开发--> 基本配置 --> Ip白名单 -中设置当前外网Ip地址为白名单
            log.info("获取access_token：" + forEntity.getBody());
            JSONObject jsonObject = JSON.parseObject(forEntity.getBody());
            Object errcode = jsonObject.get("errcode");
            if (errcode != null && !"0".equals(errcode)) {
                return R.error(RType.WX_GET_ACCESS_TOKEN_ERROR);
            }
