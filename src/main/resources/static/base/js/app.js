...
Dict = {
    ...
    getDictTest: function () {
        ...
        console.log("项目code=1:   ==>   " + Dict.convert(Enums.Dev.BaseItem, 1));
        console.log("项目code=1:  ==>   " + Dict.convertDict(Enums.Dev.BaseItem, 1));
        console.log("项目code=1:   ==>   " + Dict.convertDict(Enums.Dev.BaseItem, 1).name);
...
Ansi = {
    ...
    UnicodeToAnsi: function (chrCode) {
        ...
        var i = Ansi.UnicodeChr().indexOf(chrHex);
        if (i != -1) {
            chrHex = Ansi.AnsicodeChr().substr(i, 4);
        }
    ...
    AnsiToUnicode: function (chrCode) {
        ...
        chrHex = "000" + chrHex.toUpperCase();
        chrHex = chrHex.substr(chrHex.length - 4);
        var i = Ansi.AnsicodeChr().indexOf(chrHex);
        if (i != -1) {
            chrHex = Ansi.UnicodeChr().substr(i, 4);
        }
        return parseInt(chrHex, 16);
    },
    ...
    strUnicode2Ansi: function (asContents) {
        ...
        for (var i = 0; i < len1; i++) {
            ...
            if (varasc < 0)
                varasc += 65536;
            if (varasc > 127)
                varasc = Ansi.UnicodeToAnsi(varasc);
            if (varasc > 255) {
    ...
    strAnsi2Unicode: function (asContents) {
        ...
        for (var i = 0; i < len1; i++) {
            var varasc = asContents.charCodeAt(i);
            if (varasc > 127) {
                chrcode = Ansi.AnsiToUnicode((varasc << 8) + asContents.charCodeAt(++i));
            } else {
                chrcode = varasc;
            }
