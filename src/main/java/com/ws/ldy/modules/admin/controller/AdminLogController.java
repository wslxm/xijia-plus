...
import com.ws.ldy.modules.admin.model.vo.AdminLogVO;
import com.ws.ldy.modules.admin.model.dto.AdminLogDTO;
...
public class AdminLogController extends BaseController<AdminLogService>  {
    ...
    public R<IPage<AdminLogVO>> findPage( 
                                            @RequestParam(required = false) String fullName,
                                            @RequestParam(required = false) String uri,
                                            @RequestParam(required = false) String classDesc,
                                            @RequestParam(required = false) String methodDesc
                                              ) {
        Page<AdminLog> page = baseService.page(this.getPage(), new LambdaQueryWrapper<AdminLog>()
                .orderByDesc(AdminLog::getCreateTime)
                 .like(StringUtils.isNotBlank(fullName),AdminLog::getFullName,fullName)
                .like(StringUtils.isNotBlank(uri),AdminLog::getUri,uri)
                .like(StringUtils.isNotBlank(classDesc),AdminLog::getClassDesc,classDesc)
                .like(StringUtils.isNotBlank(methodDesc),AdminLog::getMethodDesc,methodDesc)

        );
        return R.successFind(BeanDtoVoUtil.pageVo(page, AdminLogVO.class));
    }

    @RequestMapping(value = "/findId", method = RequestMethod.GET)
    @ApiOperation(value = "ID查询", notes= "")
    public R<AdminLogVO> findId(@RequestParam String id) {
        return R.successFind(BeanDtoVoUtil.convert( baseService.getById(id),AdminLogVO.class));
    }


    @RequestMapping(value = "/insert", method = RequestMethod.POST)
    @ApiOperation(value = "添加", notes= "必须不传递ID")
    public R<Boolean> insert(@RequestBody @Validated AdminLogDTO dto) {
        if (StringUtils.isNotBlank(dto.getId())) {
            throw new ErrorException(RType.PARAM_ID_REQUIRED_FALSE);
        }
        AdminLog adminLog = dto.convert(AdminLog.class);
    ...
    public R<Boolean> upd(@RequestBody @Validated AdminLogDTO dto) {
        if (StringUtils.isBlank(dto.getId())) {
            throw new ErrorException(RType.PARAM_ID_REQUIRED_TRUE);
        }
        return R.successUpdate(baseService.updateById(dto.convert(AdminLog.class)));
    }
