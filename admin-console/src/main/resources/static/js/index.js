...
function nextMenuRoot3(data) {
    ...
    $.each(data, function (index, item) {
        ...
        if(data[index].menus.length > 0){
            ...
            html += "<dd  style='margin-left: 5%' data-name='console'><a lay-href='"+ADMIN_PATH + data[index].url+"'>"+data[index].name+"</a></dd>";
        }
    });
    return html;
}
...
function nextMenuRoot4(data) {
    ...
    $.each(data, function (index) {
        html +=  "<dd  style='margin-left: 3%'><a lay-href='"+ADMIN_PATH+data[index].url+"'>"+data[index].name+"</a></dd>";
    });
    return html;
}